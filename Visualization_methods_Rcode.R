...
# The CB.plot, landscape.view, and bimotif.view require, minimally, that you provide both a data file and the columns containing the pertinent data, while the dinculeotide.view only requires, minimally, a data file as input.
# 
# view.help()
#
...
landscape.view = function( data.file=F,  col.X=F, col.Y=F, score.type="relative",  yaxis.lim1=F, yaxis.lim2=F, headerline=T, title.plot="", save.plot.to=F, threshold=F, heuristic.plot=F , heur.resid.fold=2, heur.score.diff=0.20){
			...
			if( threshold == T | heuristic.plot == T){ 
				...
				res.fold = heur.resid.fold  # the "residual distance fold adjustment" for the regression line (shift along the y-axis)
				...
				if( ! is.na(enrichment.threshold) ){
					...
					frac.test = heur.score.diff   #  require the central count to exceed the control count by 20% of the global maximum of the set of controls with bin names greater than the bin of interest 
...
bimotif.view =function( data.file=F,  col.X=F, col.Y=F, resolution=5, yaxis.lim=F, headerline=T, title.plot="",  save.plot.to=F ){
			...
			hist( data.file[, col.X ] - data.file[, col.Y ],  breaks=1000/resolution, freq=T, cex.axis= 1,  cex.lab= 1, col="gray90", cex.main=0.8, main="", xlab="distances between motifs", ylab="frequency", xlim=xlimit, ylim=ylimit.hist )
			if( save.plot.to !=F){
				dev.off()	
			}	
...
dinucleotide.view = function( data.file=F, x1.adj=100, x2.adj=100, yaxis.lim=F, title.plot="", save.plot.to=F ){
			...
			for(ind in 1:16){
				span = seq( len*0.10, len-len*0.10, by=1)   # use span to remove alignment edges
				max.val = max(data.file[ind+1, span ]/data.file[1, span])
				max.loc = which( data.file[ind+1, ]/data.file[1,] == max.val )
				if(length(max.val)>0){
					if(max.val > 0.3 ){
						motif.loc=c(motif.loc, max.loc)  # get the maximum location for each of 16 dinucleotides
					}				
				}
			}
			if( length(motif.loc)<2){
				motif.loc=len/2;    # uncertain motif location therefore go to centre of alignment
				print("dinucleotides are not enriched in >30% of the sequences. Using middle of alignment as anchor for plotting.")
			}
			mid=round(mean(motif.loc, na.rm=T ))  # the mid of all the tallest dinucleotide positions
			...
			title(xlab="nucleotide position", ylab="dinucleotide proportion", cex.lab=1, line=2.5 )
...
#***************************
	...
	cat( "heur.resid.fold"); writeLines(strwrap( "(default = 2, i.e. 2-fold ), pertains to the enrichment zone boundary decision: the 'heur.resid.fold' generates an allowance line from the regression line by shifting the regression line y-values by default 2-fold of the 3rd largest residual value, the allowance line is used to distinguish between the zone of motif enrichment from the background. Changing 'heur.resid.fold' to <2 results in a less stringent decision of the enrichment boundary and a wider enrichment zone (to view the position of the allowance line, set parameter: heuristic.plot=T)", indent=0, exdent=0, initial="\t", prefix="\t\t\t\t") )
	cat("heur.score.diff");  writeLines(strwrap("(default = 0.20,  i.e. 20%), pertains to the motif score threshold decision: 'heur.score.diff' is the required minimal difference between the count of scores in the enrichment zone versus the count of scores in the control region to set a PWM score threshold. Increasing 'heur.score.diff' to >0.20 results in a more stringent decision of the PWM motif score threshold (to view the enrichment zone counts versus the control region counts, set parameter: heuristic.plot=T)", indent=0, exdent=0, initial="\t", prefix="\t\t\t\t") )
	cat("", sep="\n")
